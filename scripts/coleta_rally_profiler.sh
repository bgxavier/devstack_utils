...
/opt/meu_rally_install/bin/rally task start /opt/openstack_benchmarks/osv_boot.yaml | egrep -o "html.*" | (sleep 30; osprofiler trace show --html `awk '{print $2}'`)  | grep -o '"started": 0, "finished": [0-9]*, "name": "total"' | egrep -o '"finished": [0-9]*' | egrep -o [0-9]* | tee -a osprofiler.csv
